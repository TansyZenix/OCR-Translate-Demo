...
if __name__ == "__main__":
    ...
    if text:
        # print(f"检测到的文本为: {text}")
        print(f"检测到的文本为: {text.replace('‘','')}")
        translated_text = translate_text(text, source_language='en',dest_language='zh')  # 目标语言更改为中文
        if translated_text:
            # print(f"翻译后的文本为: {translated_text}")

            # 设置每行显示的最大字符数 n
            n = 60
            print("翻译后的文本为:")
            print_text_with_max_width(f"翻译后的文本为: {translated_text}", n)